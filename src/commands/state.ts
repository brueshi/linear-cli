 ...
 */
const STATE_TYPE_COLORS: Record<string, (text: string) => string> = {
  backlog: chalk.gray,
    ...
    .action(async (options) => {
      try {
        ...
        if (options.team) {
          ...
          if (isJsonMode()) {
            const statesJson: StateJson[] = sorted.map(s => stateToJson(s));
            outputJson({
          ...
          const allTeamStates: Record<string, { team: { id: string; key: string; name: string }; states: StateJson[] }> = {};
