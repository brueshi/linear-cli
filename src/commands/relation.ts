...
`)
    ...
    .action(async (issueId: string, type: string, relatedId: string) => {
      try {
        ...
        console.log(chalk.green(
          `${formatIdentifier(issue.identifier)} ${typeLabel} ${formatIdentifier(relatedIssue.identifier)}`
        ));
      } catch (error) {
        if (isJsonMode()) {
  ...
  relation
    ...
    .action(async (issueId: string) => {
      try {
        ...
        if (isJsonMode()) {
          const relationsJson: RelationJson[] = await Promise.all(
            allRelations.map(r => relationToJson(r))
          );
