...
export class AgentAIClient {
     ...
     */
    async extractIssueData(input, context) {
            ...
            try {
                ...
                if (error instanceof Anthropic.APIError) {
                    ...
                    if (error.status === 500 || error.status === 502 || error.status === 503 || error.status === 504) {
                        ...
                        retryableError.status = error.status;
                        throw retryableError;
                    }
     ...
     */
    async extractUpdateData(input, issueContext, workspaceContext) {
        // Sanitize input to prevent prompt injection
        const sanitizedInput = sanitizeInput(input);
        if (!sanitizedInput) {
            throw new Error('Input is empty after sanitization');
