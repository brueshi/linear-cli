...
export class Validator {
    ...
    validate(data, context) {
        ...
        if (data.labels && data.labels.length > 0) {
            ...
            for (const labelName of data.labels) {
                const exists = context.labels.some(l => l.name.toLowerCase() === labelName.toLowerCase());
                if (exists) {
    ...
    validateTeam(data, context) {
        ...
        if (data.teamKey) {
            const team = context.teams.find(t => t.key.toUpperCase() === data.teamKey.toUpperCase());
            if (team) {
                return { teamId: team.id };
            }
            // Try partial name match
            const teamByName = context.teams.find(t => t.name.toLowerCase().includes(data.teamKey.toLowerCase()));
            if (teamByName) {
                return {
     ...
     */
    getSuggestions(result, context) {
        const suggestions = [];
        for (const error of result.errors) {
            if (error.includes('Team') && error.includes('not found')) {
                suggestions.push(`Use --team flag to specify a team: ${context.teams.map(t => t.key).join(', ')}`);
            }
