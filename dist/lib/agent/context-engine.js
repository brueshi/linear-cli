...
export class ContextEngine {
    ...
    async fetchContext() {
        ...
        const [teamsResult, projectsResult, labelsResult, issuesResult, viewer,] = await Promise.all([
            this.fetchTeams(),
            this.fetchProjects(),
            this.fetchLabels(),
    ...
    findTeamByName(name) {
        ...
        return context.teams.find(t => t.name.toLowerCase().includes(lowerName) ||
            t.key.toLowerCase() === lowerName);
    }
    /**
     * Find labels by names (case-insensitive)
     */
    findLabelsByNames(names) {
        const context = this.getCachedContext();
        if (!context)
            return [];
        const lowerNames = names.map(n => n.toLowerCase());
        return context.labels.filter(l => lowerNames.includes(l.name.toLowerCase()));
    }
    /**
    ...
    findProjectByName(name) {
        ...
        if (!context)
            return undefined;
        const lowerName = name.toLowerCase();
        return context.projects.find(p => p.name.toLowerCase().includes(lowerName));
    }
    /**
