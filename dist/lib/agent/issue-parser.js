...
export class IssueParser {
     ...
     */
    parse(extracted, context, config) {
        const warnings = [];
        // Resolve team
        ...
        const unresolvedLabels = (extracted.labels || []).filter(name => !labels.find(l => l.name.toLowerCase() === name.toLowerCase()));
     ...
     */
    resolveTeam(extracted, context, config) {
     ...
     */
    findTeamByNamePattern(pattern, context) {
        ...
        // Direct name match
        return context.teams.find(t => t.name.toLowerCase().includes(lowerPattern)) || null;
    }
    /**
     * Infer team from recent issue patterns
     */
    inferTeamFromPatterns(context) {
     ...
     */
    resolveLabels(labelNames, context) {
        const resolved = [];
        for (const name of labelNames) {
            const label = context.labels.find(l => l.name.toLowerCase() === name.toLowerCase());
            if (label) {
                resolved.push(label);
            }
        }
     ...
     */
    applyDefaults(extracted, context, config) {
     ...
     */
    inferIssueType(title, description) {
