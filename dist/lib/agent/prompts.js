...
export function buildContextPrompt(context) {
    ...
    if (context.projects.length > 0) {
        ...
        const activeProjects = context.projects.filter(p => ['started', 'planned', 'backlog'].includes(p.state));
 ...
 */
export function buildUserPrompt(input, context) {
    const lines = [];
 ...
 */
export function buildUpdatePrompt(input, issueContext, workspaceContext) {
    const lines = [];
    // Add issue context
