...
export class BatchProcessor {
    ...
    context;
    config;
    constructor(aiClient, linearClient, context, config) {
     ...
     */
    async processBatch(inputs, options = {}, onProgress) {
     ...
     */
    async processItem(input, lineNumber, options, createLabels) {
        ...
        try {
            ...
            if (extracted.labels && extracted.labels.length > 0) {
                if (createLabels) {
                    ...
                    const labelResult = await resolveOrCreateLabels(this.linearClient, extracted.labels, this.context.labels, parseResult.team.id);
                    ...
                    const resolvedLabels = this.context.labels.filter(l => extracted.labels.some(name => l.name.toLowerCase() === name.toLowerCase()));
