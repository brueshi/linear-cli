 ...
 */
export async function findIssueByIdentifier(client, identifier) {
 ...
 */
export async function findProject(client, nameOrId) {
 ...
 */
export async function getTeamKeys(client) {
    const teams = await client.teams();
 ...
 */
export async function resolveWorkflowState(client, teamId, stateName) {
    const team = await client.team(teamId);
    const states = await team.states();
    // Exact match first
    const exact = states.nodes.find(s => s.name.toLowerCase() === stateName.toLowerCase());
    if (exact) {
        return { id: exact.id, name: exact.name, type: exact.type };
    }
    // Partial match
    const partial = states.nodes.find(s => s.name.toLowerCase().includes(stateName.toLowerCase()));
    if (partial) {
