...
import { formatIdentifier, formatState, truncate, printListHeader } from '../utils/format.js';
import { isJsonMode, outputJson, outputJsonError, issueToJson, issueSearchResultToJson, ExitCodes, } from '../utils/json-output.js';
/**
...
export function registerSearchCommands(program) {
    program
        ...
        try {
            ...
            if (isJsonMode()) {
                const issuesJson = await Promise.all(issues.map(iss => issueSearchResultToJson(iss)));
