...
import { isJsonMode, outputJson, outputJsonError, documentToJson, documentSearchResultToJson, ExitCodes, } from '../utils/json-output.js';
/**
...
export function registerDocCommands(program) {
    ...
    doc
        ...
        try {
            ...
            if (isJsonMode()) {
                const docsJson = await Promise.all(docs.nodes.map(d => documentToJson(d)));
    ...
    doc
        ...
        try {
            ...
            let doc = docs.nodes.find(d => d.title.toLowerCase() === titleOrId.toLowerCase()) || docs.nodes[0];
    ...
    doc
        ...
        try {
            ...
            if (isJsonMode()) {
                const docsJson = await Promise.all(docs.nodes.map(d => documentSearchResultToJson(d)));
