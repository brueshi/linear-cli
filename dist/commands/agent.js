...
import { formatIdentifier, divider, formatSuccess } from '../utils/format.js';
import { isJsonMode, outputJson, outputJsonError, issueToJson, commentToJson, ExitCodes, } from '../utils/json-output.js';
/**
 ...
 */
function formatPreviewBox(data, teamName, projectName) {
    const lines = [];
 ...
 */
async function agentAction(input, options) {
    try {
            ...
            const result = await processor.processBatch(inputs, {
                ...
                if (itemResult.success) {
                    if (options.dryRun) {
                        ...
                        console.log(chalk.green(`[${itemResult.lineNumber}]`) +
                            ` Created ${formatIdentifier(itemResult.issueIdentifier)} - ${itemResult.extracted?.title}`);
                    }
                }
                else {
                    console.log(chalk.red(`[${itemResult.lineNumber}]`) +
        ...
        if (!teamId && context) {
            if (extracted.teamKey) {
                const team = context.teams.find(t => t.key.toUpperCase() === extracted.teamKey?.toUpperCase());
                if (team) {
                    teamId = team.id;
                    teamName = team.name;
                    teamKey = team.key;
            ...
            if (!teamId && config.defaultTeam) {
                const team = context.teams.find(t => t.key.toUpperCase() === config.defaultTeam?.toUpperCase());
                if (team) {
                    teamId = team.id;
                    teamName = team.name;
                    teamKey = team.key;
        ...
        if (projectToResolve) {
            ...
            if (context) {
                const project = context.projects.find(p => p.id === projectToResolve ||
        ...
        if (!projectId && teamId && context && !options.auto && !options.dryRun) {
            ...
            const teamProjects = context.projects.filter(p => p.teamIds.includes(teamId));
        ...
        let createdLabels = [];
        if (extracted.labels && extracted.labels.length > 0 && context) {
            const result = await resolveOrCreateLabels(linearClient, extracted.labels, context.labels, teamId);
...
- Label additions/removals
    `)
        .action(async (issueId, input, options) => {
        try {
            // 1. Get Anthropic API key
            const anthropicApiKey = await AnthropicAuthManager.getApiKey();
            if (!anthropicApiKey) {
            ...
            let extracted;
            try {
                extracted = await aiClient.extractUpdateData(input, { identifier: issue.identifier, title: issue.title, currentStatus }, context);
            }
            catch (error) {
                if (!isJsonMode()) {
                    process.stdout.write('\r' + ' '.repeat(20) + '\r');
            ...
            if (extracted.statusChange && team) {
                ...
                const newState = states.nodes.find(s => s.name.toLowerCase().includes(extracted.statusChange.toLowerCase()));
                if (newState) {
