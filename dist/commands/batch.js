...
export function registerBatchCommands(program) {
    ...
    batch
        ...
        try {
            ...
            const updateFields = [];
            if (options.status)
                updateFields.push(`status: ${options.status}`);
            if (options.priority)
                updateFields.push(`priority: ${options.priority}`);
            if (options.assignee)
                updateFields.push(`assignee: ${options.assignee}`);
            if (options.addLabel)
                updateFields.push(`add labels: ${options.addLabel}`);
            if (options.removeLabel)
                updateFields.push(`remove labels: ${options.removeLabel}`);
            if (options.project)
                updateFields.push(`project: ${options.project}`);
            if (options.cycle)
                updateFields.push(`cycle: ${options.cycle}`);
                ...
                try {
                    ...
                    if (options.addLabel || options.removeLabel) {
                        ...
                        if (options.addLabel) {
                            ...
                            const labelsToAdd = options.addLabel.split(',').map((l) => l.trim().toLowerCase());
                        ...
                        if (options.removeLabel) {
                            const labelsToRemove = options.removeLabel.split(',').map((l) => l.trim().toLowerCase());
                            const currentLabelNames = currentLabels.nodes.map(l => l.name.toLowerCase());
                            labelIds = labelIds.filter((id, index) => !labelsToRemove.includes(currentLabelNames[index]));
                        }
            ...
            for (const issueId of issueIds) {
                try {
                    ...
                    const completedState = states.nodes.find(s => s.type === 'completed' &&
                        s.name.toLowerCase().includes(stateName.toLowerCase())) || states.nodes.find(s => s.type === 'completed');
                    if (!completedState)
