...
import { isJsonMode, outputJson, outputJsonError, attachmentToJson, ExitCodes, } from '../utils/json-output.js';
/**
...
export function registerAttachmentCommands(program) {
    ...
    attachment
        ...
        try {
            ...
            if (isJsonMode()) {
                const attachmentsJson = attachments.nodes.map(a => attachmentToJson(a));
                outputJson({
    ...
    attachment
        ...
        .option('--json', 'Output in JSON format')
        .action(async (issueId, url, options) => {
        try {
            const client = await getAuthenticatedClient();
            const issue = await findIssue(client, issueId);
            if (!issue) {
