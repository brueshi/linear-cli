...
import { isJsonMode, outputJson, outputJsonError, projectToJson, ExitCodes, } from '../utils/json-output.js';
/**
...
export function registerProjectCommands(program) {
    ...
    project
        ...
        try {
            ...
            if (options.team) {
                ...
                const projectsWithTeams = await Promise.all(projects.nodes.map(async (p) => {
            ...
            if (isJsonMode()) {
                const projectsJson = await Promise.all(filteredProjects.map(p => projectToJson(p)));
    ...
    project
        ...
        try {
            ...
            let proj = projects.nodes.find(p => p.name.toLowerCase() === name.toLowerCase()) || projects.nodes[0];
            // If multiple matches and no exact match, let user select (skip in JSON mode)
            if (!isJsonMode() && projects.nodes.length > 1 && !projects.nodes.find(p => p.name.toLowerCase() === name.toLowerCase())) {
    ...
    project
        ...
        try {
            ...
            if (options.title)
                console.log(chalk.gray(`  Name: ${updatedProject.name}`));
            if (options.description !== undefined)
                console.log(chalk.gray(`  Description: ${options.description ? 'updated' : 'cleared'}`));
            if (options.state)
                console.log(chalk.gray(`  State: ${formatProjectState(updatedProject.state)}`));
            if (options.startDate)
                console.log(chalk.gray(`  Start: ${updatedProject.startDate}`));
            if (options.targetDate)
                console.log(chalk.gray(`  Target: ${updatedProject.targetDate}`));
        }
        catch (error) {
            if (isJsonMode()) {
