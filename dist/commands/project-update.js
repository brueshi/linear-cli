...
import { isJsonMode, outputJson, outputJsonError, projectUpdateToJson, ExitCodes, } from '../utils/json-output.js';
/**
 ...
 */
export function registerProjectUpdateCommands(program) {
    ...
    update
        ...
        try {
            ...
            if (isJsonMode()) {
                const updatesJson = await Promise.all(updates.nodes.map(u => projectUpdateToJson(u)));
                outputJson({
