...
import { formatIdentifier, formatPriority, truncate } from '../utils/format.js';
        ...
        try {
            ...
            if (isJsonMode()) {
                ...
                if (showAll || options.assigned) {
                    ...
                    result.assigned = await Promise.all(activeIssues.nodes.map(i => issueToJson(i)));
                }
                if (options.created) {
                    ...
                    result.created = await Promise.all(createdIssues.nodes.map(i => issueToJson(i)));
                }
                if (showAll || options.due) {
                    ...
                    result.due = await Promise.all(dueIssues.nodes.map(i => issueToJson(i)));
                }
                if (showAll) {
 ...
 */
async function showAssignedIssues(client, userId, limit) {
 ...
 */
async function showCreatedIssues(client, userId, limit) {
 ...
 */
async function showDueSoon(client, userId, limit) {
    ...
    for (const issue of dueIssues.nodes) {
        ...
        console.log(`  ${formatIdentifier(issue.identifier.padEnd(10))} ${truncate(issue.title, 40).padEnd(42)} ${dueStr}`);
    }
    console.log('');
}
/**
 ...
 */
async function showSummaryStats(client, userId) {
 ...
 */
async function printIssueLine(issue) {
    const priority = formatPriority(issue.priority);
    console.log(`  ${formatIdentifier(issue.identifier.padEnd(10))} ${priority.padEnd(12)} ${truncate(issue.title, 45)}`);
}
